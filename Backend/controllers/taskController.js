...
const createTask = async (req, res) => {
    try {
        ...
        if(!Array.isArray(assignedTo)){
            return res.status(400).json({message: "Invalid assignedTo array of user IDs"});
        }
...
const updateTask = async (req, res) => {
    try {
       ...
       task.sescription = req.body.description || task.description;
       task.priority = req.body.priority || task.priority;
       task.dueDate = req.body.dueDate || task.dueDate;
       task.todoCheckList = req.body.todoCheckList ||  task.todoCheckList;
       task.attachments = req.body.attachments || task.todoCheckList;

       if(req.body.assignedTo){
        if(!Arrsy.isArray(req.body.assignedTo)) {
            return res
            .status(400)
            .json({ message: "assignedTo must be an array of users."});
        }
...
const updateTaskChecklist = async (req, res) => {
    try {
        ...
        res.json({ message: "Task checklist completed" , task:updateTask});
...
const getDashboardData = async (req, res) => {
    try {
        ...
        const taskPriotyLevels = taskPriorities.reduce((acc, priority) => {
        ...
        .sort({ createdAt: -1})
        .limit(10)
        .select("Title status priority sueDate createdAt");
...
const getUserDashboardData = async (req, res) => {
    try {
        const userId = req.user._id; // only fetch data for loggefd in user

        //Fetch ststistics for user specific tasks
        ...
        //Fetch recent 10 tasks by assigned users
